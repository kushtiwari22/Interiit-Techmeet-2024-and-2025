...
class PPOMemory:
    ...
    def generate_batches(self):
        ...
        # FIX: Return early if no states
        if n_states == 0:
            # Return empty tensors with proper shape
            return (
                T.tensor([], dtype=T.float32).reshape(0, 0),  # Empty 2D tensor
        ...
        # FIX: Ensure states array is properly shaped
        states_array = np.array(self.states, dtype=np.float32)
        if states_array.ndim == 1:
            # If states are 1D, reshape to 2D (n_states, 1)
            states_array = states_array.reshape(-1, 1)
        
        # FIX: Check if states array has valid shape
        if states_array.shape[0] == 0 or states_array.shape[1] == 0:
            return (
                T.tensor([], dtype=T.float32).reshape(0, 0),
                T.tensor([], dtype=T.float32),
                T.tensor([], dtype=T.float32),
                T.tensor([], dtype=T.float32),
                T.tensor([], dtype=T.float32),
        ...
        return (
            T.tensor(states_array, dtype=T.float32),
            T.tensor(np.array(self.actions), dtype=T.float32),
    ...
    def store_memory(self, state, action, probs, vals, reward, done):
        # FIX: Ensure state is properly formatted before storing
...
class CriticNetwork(nn.Module):
    def __init__(
        ...
        # FIXED: Removed Dropout layers from critic for better learning
        self.critic = nn.Sequential(
            nn.Linear(input_dims, fc1_dims),
            nn.LayerNorm(fc1_dims),
            nn.ReLU(),
            # nn.Dropout(0.1),  # COMMENTED OUT - was harming critic learning
            nn.Linear(fc1_dims, fc2_dims),
            nn.LayerNorm(fc2_dims),
            nn.ReLU(),
            # nn.Dropout(0.1),  # COMMENTED OUT - was harming critic learning
...
class Agent:
    def __init__(
        ...
        # FIX: Store input_dims for validation
    ...
    def learn(self):
        """
        PPO learn with the following FIXES:
        1. Added validation for empty memory
        2. Added shape validation for states
        3. Fixed critic input dimension mismatch
        """
        self.learn_step += 1
        ...
        # FIX: Check if state_arr is empty or has invalid shape
        if len(state_arr) == 0 or state_arr.shape[1] == 0:
            print(f"[SKIP] Learn Step {self.learn_step}: No valid data in memory")
            print(f"State shape: {state_arr.shape}")
            self.memory.clear_memory()
            return
        
        # FIX: Check if states have correct number of features
        if state_arr.shape[1] != self.input_dims:
            print(f"[SKIP] Learn Step {self.learn_step}: State feature mismatch")
            print(f"Expected {self.input_dims} features, got {state_arr.shape[1]}")
            print(f"State shape: {state_arr.shape}")
            self.memory.clear_memory()
            return
        ...
        # Check if advantages are non-trivial
        # FIXED: Skip update if advantages are too small
        if advantages.std() < 1e-6:
            print(f"[SKIP] Learn Step {self.learn_step}: advantages.std() too small ({advantages.std():.6f})")
            self.memory.clear_memory()
            return
        ...
        for epoch in range(self.n_epochs):
            for batch in batches:
                ...
                # FIXED: Compute critic loss explicitly
                ...
                # FIXED: Loosen gradient clipping for critic (more important now)
        ...
        if self.learn_step % 10 == 0 and batch_count > 0:
            ...
            # FIXED: Debug critic gradients
            for name, param in self.critic.named_parameters():
                if param.grad is not None:
                    grad_norm = param.grad.abs().mean().item()
                    print(f"  [DEBUG] Critic grad '{name}': {grad_norm:.6f}")
                    break  # Just print first layer for brevity